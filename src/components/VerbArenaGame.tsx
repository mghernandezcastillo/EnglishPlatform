...
export type LexiconCategory = 'common_verb' | 'irregular_verb' | 'phrasal_verb' | 'idiom';

export type ArenaChallengeType = 'meaning' | 'conjugation' | 'listening' | 'vocab_vault' | 'warmup';

export type VerbGuideItem = {
...
export type GameItem = VerbGuideItem & {
  answer: string;
...
export type Round = {
  item: GameItem;
  options: string[];
  challengeType: 'meaning' | 'conjugation' | 'listening';
  correctAnswer: string;
};

export type GameMode = 'all' | LexiconCategory;
export type Phase = 'intro' | 'countdown' | 'question' | 'feedback' | 'results';
export type Feedback = 'correct' | 'wrong' | 'unknown' | 'timeout';
...
const arenaTabs: { id: ArenaChallengeType; label: string; icon: any; detail: string }[] = [
  { id: 'meaning', label: 'Significado', icon: Zap, detail: 'Término ➔ Significado' },
  { id: 'conjugation', label: 'Past & Participle', icon: Flame, detail: 'Base ➔ Past / Participle' },
  { id: 'listening', label: 'Audio Trainer', icon: Headphones, detail: 'Escucha y Reconoce' },
  { id: 'vocab_vault', label: 'Mis Verbos Débiles', icon: BookmarkPlus, detail: 'Sync con VocabVault' },
  { id: 'warmup', label: 'Glosario Rápido', icon: BookOpen, detail: 'Estudia antes de jugar' },
];

const modes: { id: GameMode; label: string; detail: string }[] = [
  { id: 'all', label: 'Mixed Challenge', detail: 'Todo mezclado' },
  { id: 'common_verb', label: 'Common Verbs', detail: 'Verbos más usados' },
  { id: 'irregular_verb', label: 'Irregular Verbs', detail: 'Verbos irregulares' },
  { id: 'phrasal_verb', label: 'Phrasal Verbs', detail: 'Verbos compuestos' },
  { id: 'idiom', label: 'Idioms & Expresiones', detail: 'Modismos reales' },
];
...
function getRoundKey(item: VerbGuideItem, challenge: string = 'meaning') {
  return `${challenge}:${item.category}:${cleanDisplayText(item.term).toLowerCase()}:${cleanAnswer(item).toLowerCase()}`;
}
...
const COMMON_VERB_CONJUGATIONS: Record<string, { past: string; participle: string }> = {
  ...
  melt: { past: 'melted', participle: 'melted' },
...
function toGameItems(items: (VerbGuideItem | any)[]): GameItem[] {
  return items
    .map((item) => {
      ...
      const pastStr = forms.past ? cleanDisplayText(forms.past) : '';
      const partStr = forms.past_participle ? cleanDisplayText(forms.past_participle) : '';
      const conj = pastStr && partStr ? `${pastStr} / ${partStr}` : pastStr || partStr || undefined;
...
function buildRound(
  pool: GameItem[],
  mode: GameMode,
  usedRoundKeys: Set<string>,
  challengeType: 'meaning' | 'conjugation' | 'listening' = 'meaning'
): Round | null {
  if (pool.length === 0) return null;

  const validPool = challengeType === 'conjugation'
    ? pool.filter((i) => Boolean(i.conjugationAnswer))
    : pool;

  const activePool = validPool.length > 0 ? validPool : pool;
  const availableItems = activePool.filter((candidate) => !usedRoundKeys.has(getRoundKey(candidate, challengeType)));
  if (availableItems.length === 0) return null;

  const item = availableItems[Math.floor(Math.random() * availableItems.length)];

  if (challengeType === 'conjugation' && item.conjugationAnswer) {
    const correctAnswer = item.conjugationAnswer;
    const distractorSource = activePool
      .filter((candidate) => candidate.conjugationAnswer && candidate.conjugationAnswer !== correctAnswer)
      .map((c) => c.conjugationAnswer!);

    let distractors = shuffle(Array.from(new Set(distractorSource))).slice(0, 3);
    while (distractors.length < 3) {
      distractors.push(`${item.term}ed / ${item.term}ed`);
    }
  ...
  const distractorSource = pool.filter(
    (candidate) => candidate.answer !== item.answer && (mode === 'all' || candidate.category === item.category)
  );
  ...
  return {
    item,
    options: shuffle([correctAnswer, ...rawDistractors]),
    challengeType: challengeType === 'listening' ? 'listening' : 'meaning',
    correctAnswer,
  };
}
...
export interface VerbArenaGameProps {
  onBack?: () => void;
  isEmbedded?: boolean;
  customPool?: any[];
  maxRounds?: number;
...
export function VerbArenaGame({
  ...
  const [activeTab, setActiveTab] = useState<ArenaChallengeType>('meaning');
  const [items, setItems] = useState<GameItem[]>([]);
  const [vaultItems, setVaultItems] = useState<GameItem[]>([]);
  ...
  useEffect(() => {
    if (activeTab === 'vocab_vault') {
      setLoadingVault(true);
      vocabService
        .getItems(activeStudentId)
        .then((vault) => {
          if (vault && vault.length > 0) {
            const formatted = vault.map((v) => ({
              category: 'common_verb' as LexiconCategory,
              term: v.term,
              base_verb: v.term,
              meaning_es: v.meanings?.[0]?.meaningLabel || v.meanings?.[0]?.definitionEs || v.translation || '',
              example_en: v.meanings?.[0]?.contextExamples?.[0]?.en || null,
              answer: v.meanings?.[0]?.meaningLabel || v.meanings?.[0]?.definitionEs || v.translation || '',
            }));
            setVaultItems(toGameItems(formatted));
          } else {
            setVaultItems([]);
          }
        })
  ...
  const pool = useMemo(() => {
    if (activeTab === 'vocab_vault') {
      return vaultItems.length > 0 ? vaultItems : items;
    }
    if (activeTab === 'conjugation') {
      const conjPool = items.filter((i) => Boolean(i.conjugationAnswer));
      if (mode === 'all') return conjPool;
      return conjPool.filter((item) => item.category === mode);
    }
    if (mode === 'all') return items;
    return items.filter((item) => item.category === mode);
  }, [items, vaultItems, mode, activeTab]);

  const filteredWarmupItems = useMemo(() => {
    const q = warmupSearch.trim().toLowerCase();
    if (!q) return pool.slice(0, 100);
    return pool.filter(
      (item) =>
        item.term.toLowerCase().includes(q) ||
        (item.meaning_es && item.meaning_es.toLowerCase().includes(q)) ||
        (item.past && item.past.toLowerCase().includes(q))
    ).slice(0, 100);
  }, [pool, warmupSearch]);
  ...
  const prepareRound = () => {
    const targetLimit = maxRounds || pool.length;
    if (stats.total >= targetLimit) {
      setPhase('results');
      return;
    }
    ...
    const nextRound = buildRound(
      ...
      mode,
      usedRoundKeys,
      activeTab === 'conjugation' ? 'conjugation' : activeTab === 'listening' ? 'listening' : 'meaning'
    );
    ...
    setRound(nextRound);
    setUsedRoundKeys((prev) => new Set(prev).add(getRoundKey(nextRound.item, nextRound.challengeType)));
    setSelectedAnswer(null);
    setFeedback(null);
    ...
    if (nextRound.challengeType === 'listening') {
      playAudio(nextRound.item.term);
    }
  };

  const answerRound = (selected: string | null, status: Feedback) => {
    if (!round || feedback) return;
    setSelectedAnswer(selected);
    setFeedback(status);
    playSound(status === 'correct' ? 'correct' : 'wrong');
    ...
    setStats((prev) => ({
      ...
      timeout: prev.timeout + (status === 'timeout' ? 1 : 0),
    }));
  ...
  const handleAnswer = (selected: string) => {
    ...
    answerRound(selected, isCorrect ? 'correct' : 'wrong');
  };

  useEffect(() => {
    if (phase !== 'countdown') return;
    if (lastCountdownSoundRef.current !== countdown) {
  ...
  }, [phase, countdown, pool, mode, usedRoundKeys, maxRounds, activeTab]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        if (prev <= 3 && lastTimerSoundRef.current !== prev) {
          lastTimerSoundRef.current = prev;
          playSound('danger');
        } else if (lastTimerSoundRef.current !== prev) {
          lastTimerSoundRef.current = prev;
          playSound('timer');
        }
        return prev - 1;
    ...
    return () => clearInterval(timer);
  }, [phase, timerEnabled, round, feedback]);

  const shareResults = () => {
    const lines = [
      `⚡ *AI Verb Arena Report - Maven English*`,
      `🎯 Modalidad: ${arenaTabs.find((t) => t.id === activeTab)?.label || 'Significado'}`,
      `📊 Precisión: ${accuracy}% (${stats.correct}/${stats.total})`,
      `⏱️ Tiempo por pregunta: ${timerSeconds > 0 ? `${timerSeconds}s` : 'Sin límite'}`,
      '',
      missedHistory.length ? '📝 Palabras para reforzar:' : '✅ ¡Dominio perfecto de todos los verbos!',
      ...missedHistory.map((entry, index) => `${index + 1}. ${formatShareEntry(entry.item)}`),
      '',
  ...
  const practiceMissed = () => {
    ...
    if (missedItems.length < 2) return;
  ...
  return (
    <div className={isEmbedded ? 'w-full h-full text-white flex flex-col justify-between overflow-hidden' : 'min-h-screen bg-slate-950 text-white'}>
      ...
      {!isEmbedded && (
        <header className="relative z-20 flex flex-col gap-3 px-4 py-4 sm:px-6 border-b border-white/10 bg-slate-950/70 backdrop-blur-xl">
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              {onBack && (
                <button
                  onClick={onBack}
                  className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-white/15 bg-white/10 text-white shadow-lg backdrop-blur transition hover:bg-white/20 cursor-pointer"
                  aria-label="Volver"
                >
            ...
            {phase !== 'intro' && (
              <button
                ...
                className="rounded-2xl border border-white/15 bg-white/10 px-4 py-2.5 text-sm font-black text-white shadow-lg backdrop-blur transition hover:bg-white/20 cursor-pointer"
              >
                Ver Resultados
              </button>
            )}
            ...
            {arenaTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-black transition-all cursor-pointer border ${
                    isActive
                      ? 'bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 border-cyan-300 shadow-lg shadow-cyan-500/20 scale-105'
                      : 'bg-white/5 hover:bg-white/10 text-slate-200 border-white/10 hover:border-white/20'
                  }`}
                >
                  <Icon className={`w-4 h-4 ${isActive ? 'text-slate-950 stroke-[2.5]' : 'text-cyan-300'}`} />
                  <span>{tab.label}</span>
                </button>
              );
            })}
      ...
      <main
        className={
          isEmbedded
            ? 'relative z-10 w-full h-full flex flex-col justify-between p-3 sm:p-5 overflow-hidden'
            : 'relative z-10 mx-auto flex min-h-[calc(100vh-140px)] max-w-7xl items-center px-4 pb-6 sm:px-6'
        }
      >
        <AnimatePresence mode="wait">
          {phase === 'intro' && (
            ...
            >
              <div className={isEmbedded ? 'w-full lg:w-[48%] flex flex-col justify-between py-1 min-w-0' : 'max-w-4xl'}>
                <div>
                  ...
                  >
                    ...
                    ⚡ {arenaTabs.find((t) => t.id === activeTab)?.label || 'AI Vocabulary Challenge'}
                  </motion.div>

                  <h1
                    className={
                      isEmbedded
                        ? 'text-5xl sm:text-6xl lg:text-[4.5rem] font-black leading-[0.92] tracking-tight text-white drop-shadow-md'
                        : 'text-5xl font-black leading-[0.9] tracking-tight sm:text-7xl lg:text-8xl'
                    }
                  >
                    AI Verb
                  ...
                  <p
                    className={
                      isEmbedded
                        ? 'mt-3 text-lg sm:text-xl lg:text-2xl font-bold leading-snug text-slate-200'
                        : 'mt-5 max-w-2xl text-xl font-semibold leading-8 text-slate-200 sm:text-2xl'
                    }
                  >
                    {activeTab === 'conjugation'
                      ? 'Entrena tu dominio de formas verbales (Simple Past & Past Participle) con retroalimentación instantánea.'
                      : activeTab === 'listening'
                      ? 'Entrena tu comprensión auditiva escuchando pronunciación nativa de alta fidelidad.'
                      : activeTab === 'vocab_vault'
                      ? 'Desafío personalizado enfocado exclusivamente en tus palabras guardadas de clase.'
                      : activeTab === 'warmup'
                      ? 'Revisa el catálogo interactivo de verbos con audio y fonética antes de iniciar.'
                      : 'Trivia interactiva de alta velocidad para dominar verbos y expresiones en inglés.'}
                  </p>
                </div>
                ...
                {isEmbedded ? (
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-bold text-slate-300">
                      <Zap className="h-4 w-4 text-amber-400" />
                      <span>{pool.length} Verbos en Catálogo</span>
                    </div>
                    <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-bold text-slate-300">
              ...
              {activeTab === 'warmup' ? (
                <div className="rounded-[2rem] border border-white/15 bg-slate-950/85 p-5 shadow-2xl backdrop-blur-xl flex flex-col h-[520px]">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2 text-base font-black text-cyan-200">
                    ...
                    <span className="text-xs font-mono font-bold text-cyan-300 bg-cyan-400/10 px-2.5 py-1 rounded-lg border border-cyan-400/20">
                      {filteredWarmupItems.length} Verbos
                    </span>
                  </div>
                  ...
                  <div className="relative mb-3">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-white/15 bg-white/5 pl-10 pr-4 py-2.5 text-sm font-bold text-white placeholder:text-slate-400 focus:border-cyan-400 focus:outline-none"
                    />
                  </div>

                  <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                    {filteredWarmupItems.map((item, idx) => (
                      <div key={idx} className="rounded-2xl border border-white/10 bg-white/5 p-3 flex items-center justify-between gap-2 hover:bg-white/10 transition">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-base font-black text-white">{item.term}</span>
                            <button
                              onClick={() => playAudio(item.term)}
                              className="p-1 rounded-lg bg-cyan-400/15 hover:bg-cyan-400/30 text-cyan-300 transition cursor-pointer"
                              title="Escuchar pronunciación"
                            >
                              <Volume2 className="h-3.5 w-3.5" />
                            </button>
                          </div>
                          <p className="text-xs font-bold text-cyan-300 mt-0.5">{item.answer}</p>
                          {item.past && (
                            <p className="text-[11px] font-mono text-slate-400 mt-0.5">
                              Past: {item.past} • Participle: {item.past_participle || '-'}
                            </p>
                          )}
                        </div>
                        <button
                          onClick={() => handleSaveToVocabVault(item)}
                          className="shrink-0 p-2 rounded-xl bg-white/10 hover:bg-amber-500 hover:text-slate-950 text-white transition cursor-pointer"
                          title="Guardar en VocabVault"
                        >
                  ...
                  <button
                    ...
                    className="mt-3 flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-cyan-300 via-white to-pink-300 px-6 py-3 text-lg font-black text-slate-950 shadow-xl transition hover:scale-[1.02] cursor-pointer"
                  >
                    <Play className="h-5 w-5 fill-current" />
                    ¡Iniciar Arena con estos Verbos! 🚀
                  </button>
                </div>
              ) : (
                <div
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-36 overflow-y-auto">
                          {pool.map((item, idx) => (
                            <div key={idx} className="rounded-xl bg-cyan-400/15 border border-cyan-400/40 px-3 py-2 text-xs sm:text-sm font-black text-cyan-100 flex items-center justify-between shadow-sm">
                              ...
                              <span className="text-cyan-300 text-xs truncate ml-1">➔ {item.answer}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                          ...
                          <button
                            ...
                            className={`h-11 sm:h-13 rounded-2xl text-sm sm:text-base font-black transition cursor-pointer ${
                              timerSeconds === option.value
                                ? 'bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 font-black shadow-lg scale-[1.03] ring-2 ring-cyan-300'
          ...
          {phase === 'countdown' && (
            <motion.section
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={isEmbedded ? 'grid w-full place-items-center h-full my-auto' : 'grid min-h-[calc(100vh-140px)] w-full place-items-center'}
            >
              <motion.div
                key={countdown}
              ...
              >
                <div className="px-4">
                  <div className={isEmbedded ? 'text-xl font-black uppercase tracking-[0.2em] text-cyan-200' : 'text-2xl font-black uppercase tracking-[0.28em] text-cyan-100 sm:text-4xl'}>
                    Prepárate
                  </div>
                  <div
                    className={
                      isEmbedded
                        ? 'text-9xl font-black leading-none text-white drop-shadow-[0_0_36px_rgba(103,232,249,0.85)] my-2'
                        : 'text-[34vw] font-black leading-none text-white drop-shadow-[0_0_36px_rgba(103,232,249,0.85)] sm:text-[22rem]'
                    }
                  >
                    {Math.max(countdown, 1)}
                  </div>
                  <div className={isEmbedded ? 'text-base font-black uppercase tracking-[0.2em] text-pink-300' : 'text-xl font-black uppercase tracking-[0.2em] text-red-100 sm:text-3xl'}>
                    Get ready
                  </div>
                </div>
              </motion.div>
          ...
          {phase === 'question' && round && (
            <motion.section
              key={round.item.term + round.challengeType}
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -24 }}
              className={isEmbedded ? 'w-full h-full flex flex-col justify-between py-1' : 'w-full'}
            >
              <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <div className={`rounded-2xl bg-gradient-to-r ${categoryAccent[round.item.category]} px-5 py-2 text-sm sm:text-base font-black uppercase tracking-wider text-slate-950 shadow-md`}>
                    {round.challengeType === 'conjugation'
                      ? '⚡ Past & Participle'
                      : round.challengeType === 'listening'
                      ? '🎧 Audio Trainer'
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className={`relative grid h-16 w-16 place-items-center rounded-full border ${
                      timeLeft <= 3 && timerEnabled ? 'border-red-300 text-red-100' : 'border-cyan-200/40 text-white'
                    } bg-slate-950/60 shadow-lg backdrop-blur`}
                  >
                    <div
                      className="absolute inset-1 rounded-full"
                      style={{
                        background: `conic-gradient(${timeLeft <= 3 && timerEnabled ? '#fb7185' : '#67e8f9'} ${timerProgress}%, rgba(255,255,255,0.12) 0)`,
                      }}
                    />
              ...
              >
                <div className="text-center">
                  <div
                    className={
                      isEmbedded
                        ? 'mb-1 text-xs sm:text-sm font-black uppercase tracking-[0.25em] text-cyan-300'
                        : 'mb-3 text-base font-black uppercase tracking-[0.25em] text-cyan-100'
                    }
                  >
                    {round.challengeType === 'conjugation'
                      ? '¿Cuál es el Simple Past y Past Participle correcto?'
                      : round.challengeType === 'listening'
                      ? '🎧 Escucha y selecciona la opción correcta'
                  ...
                  {round.challengeType === 'listening' ? (
                    <div className="my-4 flex flex-col items-center">
                      <button
                        onClick={() => playAudio(round.item.term)}
                        className="flex items-center gap-3 px-8 py-4 rounded-3xl bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 font-black text-2xl shadow-xl shadow-cyan-500/30 hover:scale-105 active:scale-95 transition cursor-pointer"
                      >
                        <Volume2 className="h-8 w-8 stroke-[2.5]" />
                        <span>🔊 Escuchar Verbo</span>
                      </button>
                      <p className="text-xs font-bold text-slate-300 mt-2">Haz clic para repetir el audio</p>
                    </div>
                  ) : (
                    <h2
                      className={
                  ...
                  {round.challengeType === 'meaning' && round.item.past && (
                    <div
                      className={
                        isEmbedded
                          ? 'mt-2 inline-flex flex-wrap justify-center gap-2 rounded-2xl bg-white/10 p-1.5 text-xs sm:text-sm font-black text-slate-200 border border-white/15'
                          : 'mt-5 inline-flex flex-wrap justify-center gap-2 rounded-2xl bg-white/8 p-2 text-sm font-black text-slate-200 sm:text-base'
                      }
                    >
          ...
          {phase === 'feedback' && round && feedback && (
            <motion.section
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.98 }}
              className={isEmbedded ? 'w-full h-full flex flex-col justify-between' : 'w-full'}
            >
              <div
                className={
                  isEmbedded
                    ? `rounded-3xl border-2 p-5 sm:p-7 shadow-2xl backdrop-blur-xl flex flex-col justify-between h-full ${
                        feedback === 'correct' ? 'border-emerald-300/50 bg-emerald-950/80 shadow-emerald-900/40' : 'border-red-300/50 bg-red-950/80 shadow-red-900/40'
                      }`
                    : `rounded-[2rem] border p-5 shadow-2xl backdrop-blur-xl sm:p-7 lg:p-9 ${
                        feedback === 'correct' ? 'border-emerald-200/40 bg-emerald-400/16 shadow-emerald-900/30' : 'border-red-200/35 bg-red-400/14 shadow-red-900/30'
                      }`
                }
              >
                <div>
                  ...
                  >
                    {feedback === 'correct' ? (
                      <CheckCircle2 className={isEmbedded ? 'h-10 w-10 text-emerald-600' : 'h-14 w-14 text-emerald-600'} />
                    ) : (
                      <XCircle className={isEmbedded ? 'h-10 w-10 text-red-600' : 'h-14 w-14 text-red-600'} />
                    )}
                  </motion.div>
                  ...
                  <div className={isEmbedded ? 'mx-auto mt-3 grid max-w-5xl gap-3 sm:grid-cols-2' : 'mx-auto mt-6 grid max-w-5xl gap-4 lg:grid-cols-[0.9fr_1.1fr]'}>
                    <div className={isEmbedded ? 'rounded-2xl border-2 border-white/20 bg-slate-950/80 p-4 text-left shadow-lg' : 'rounded-3xl border border-white/10 bg-slate-950/52 p-5 text-left'}>
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => playAudio(round.item.term)}
                          className="p-1 rounded-lg bg-cyan-400/20 hover:bg-cyan-400/40 text-cyan-200 transition cursor-pointer"
                    ...
                    <div className={isEmbedded ? 'rounded-2xl bg-white p-4 text-left text-slate-950 shadow-lg border-2 border-white' : 'rounded-3xl bg-white p-5 text-left text-slate-950'}>
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Respuesta Correcta</p>
                      <p className={isEmbedded ? 'mt-1 break-words text-2xl sm:text-3xl font-black leading-snug' : 'mt-2 break-words text-3xl font-black leading-tight sm:text-4xl'}>
                        {round.correctAnswer}
                      </p>
                      {selectedAnswer && selectedAnswer !== round.correctAnswer && (
                  ...
                  {getDisplayExample(round.item) && (
                    <div
                      className={
                        isEmbedded
                          ? 'mx-auto mt-3 max-w-5xl rounded-2xl border-2 border-cyan-400/40 bg-cyan-950/60 p-3.5 text-left shadow-md'
                          : 'mx-auto mt-4 max-w-5xl rounded-3xl border border-cyan-200/20 bg-cyan-300/10 p-5 text-left'
                      }
                    >
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Ejemplo de uso</p>
                      <p className={isEmbedded ? 'mt-1 text-base sm:text-lg font-bold leading-snug text-white' : 'mt-2 text-2xl font-bold leading-snug text-white'}>
                        {getDisplayExample(round.item)}
                      </p>
                    </div>
                  )}
              ...
              {isEmbedded ? (
                ...
                <div className="rounded-[2rem] border border-white/15 bg-slate-950/80 p-4 shadow-2xl backdrop-blur-xl sm:p-6 lg:p-8">
                  <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-3 max-w-2xl text-lg font-semibold leading-7 text-slate-300">
                        Feedback con las palabras practicadas y áreas de oportunidad.
                      </p>
                      {statusMessage && (
                      ...
                      <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                        <div className="rounded-3xl bg-white p-5 text-slate-950">
                          ...
                          {missedHistory.length ? (
                            <div className="grid gap-3 xl:grid-cols-2 max-h-[340px] overflow-y-auto">
                              {missedHistory.map((entry, index) => (
                                <div key={`${entry.item.term}-${entry.status}-${index}`} className="rounded-2xl bg-slate-50 p-4">
                                  ...
                                  <div className="mt-2 text-base font-black leading-snug text-slate-700">
                                    Correcta: {entry.correctAnswer}
                                  </div>
                                  {entry.selectedAnswer && (
                                    <div className="mt-2 text-sm font-bold leading-snug text-red-700">
                                      Tu respuesta: {cleanDisplayText(entry.selectedAnswer)}
                                    </div>
                                  )}
                                </div>
                              ))}
                        ...
                        <div className="rounded-3xl border border-cyan-200/20 bg-cyan-300/10 p-5">
                          ...
                          <button
                            ...
                            className="mt-5 min-h-16 w-full rounded-2xl bg-emerald-500 px-5 py-4 text-lg font-black text-white shadow-lg shadow-emerald-950/30 transition hover:bg-emerald-600 cursor-pointer"
                          >
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-3">
                    <button onClick={startGame} className="min-h-16 rounded-2xl bg-white px-5 py-4 text-lg font-black text-slate-950 transition hover:bg-cyan-50 cursor-pointer">
                      <RotateCcw className="mr-2 inline h-5 w-5" />
                      Play Again
                    </button>
                    <button
                      onClick={practiceMissed}
                      disabled={missedHistory.length < 2}
                      className="min-h-16 rounded-2xl bg-cyan-300 px-5 py-4 text-lg font-black text-slate-950 transition hover:bg-cyan-200 disabled:opacity-50 cursor-pointer"
                    >
                      Practice missed
                    </button>
                    <button onClick={shareResults} className="min-h-16 rounded-2xl bg-emerald-500 px-5 py-4 text-lg font-black text-white transition hover:bg-emerald-600 cursor-pointer">
                      <MessageCircle className="mr-2 inline h-5 w-5" />
                      WhatsApp
                    </button>
                  </div>
