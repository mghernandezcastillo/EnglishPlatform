                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        ...
                        <div className="grid gap-3 p-4 sm:grid-cols-2 sm:p-6 lg:grid-cols-3">
                          {block.lessons.map((lesson) => {
                            ...
                            const started = !complete && lesson.stories.some((story) => (progress.lineByStory[story.story_id] || 0) > 0);
                            return (
                              <button key={lesson.lesson_id} type="button" disabled={!unlocked} onClick={() => openLesson(block, lesson)} className={`group flex min-h-32 items-center gap-4 rounded-2xl border-2 p-4 text-left transition ${complete ? 'border-emerald-200 bg-emerald-50 hover:border-emerald-400' : started ? 'border-amber-200 bg-amber-50 hover:-translate-y-1 hover:border-amber-400 hover:shadow-xl' : unlocked ? 'border-indigo-100 bg-white hover:-translate-y-1 hover:border-indigo-300 hover:shadow-xl' : 'border-slate-100 bg-slate-100 text-slate-400'}`}>
                                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl font-black ${complete ? 'bg-emerald-500 text-white' : started ? 'bg-amber-400 text-amber-950' : unlocked ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-400'}`}>{complete ? <Check className="h-6 w-6" /> : started ? <RefreshCw className="h-5 w-5" /> : unlocked ? lesson.lesson_id : <LockKeyhole className="h-5 w-5" />}</div>
                                <div className="min-w-0 flex-1">
                                  <div className={`text-[0.65rem] font-black uppercase tracking-[0.17em] ${complete ? 'text-emerald-700' : started ? 'text-amber-700' : 'text-indigo-500'}`}>Clase {lesson.lesson_id} · {completedStories}/3 historias · {complete ? 'Completada' : started ? 'En progreso' : 'Nueva'}</div>
